...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class The_Pasonanca_Park : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_the_pasonanca_park);
            ...
            {
                Resource.Drawable.the_pasonanca_park_1,
                Resource.Drawable.the_pasonanca_park_2,
                Resource.Drawable.the_pasonanca_park_3,
                Resource.Drawable.the_pasonanca_park_4,
            };
            ...
            else
            {
                favoritesItem = new FavoriteItem
                {
                    ...
                    Description = "It is a popular tourist attraction located in Zamboanga City, Philippines. There is no entrance fee to visit the tree house, but visitors are encouraged to make a donation to help maintain the park. "
                };
            }
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "the_pasonanca_park_1" && cm.Title == "The Pasonanca Park" && cm.Description == "It is a popular tourist attraction located in Zamboanga City, Philippines. There is no entrance fee to visit the tree house, but visitors are encouraged to make a donation to help maintain the park. ");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("TPP").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("TPPNonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("TPP");
