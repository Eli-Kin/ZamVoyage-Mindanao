...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class Merloquet_Falls : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_merloquet_falls);
            ...
            {
                Resource.Drawable.merloquet_falls_1,
                Resource.Drawable.merloquet_falls_2,
                Resource.Drawable.merloquet_falls_3,
                Resource.Drawable.merloquet_falls_4,
            };
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "merloquet_falls_1" && cm.Title == "Merloquet Falls" && cm.Description == "It is a stunning waterfall that is located in Barangay Sibulao, Zamboanga City, Philippines. The waterfall is situated in a lush jungle setting, surrounded by towering cliffs and dense vegetation.");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("MF").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("MFNonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("MF");
