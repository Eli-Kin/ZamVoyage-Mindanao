...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class Curacha : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_curacha);
            ...
            {
                Resource.Drawable.curacha_1,
                Resource.Drawable.curacha_2,
                Resource.Drawable.curacha_3,
                Resource.Drawable.curacha_4,
            };
            ...
            else
            {
                favoritesItem = new FavoriteItem
                {
                    ImagePath = "curacha_1",
                    Title = "Curacha",
                    Description = "Curacha got its name because of its spiky, hairy appearance that kinda resembles you-know-what."
                };
            }
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "curacha _1" && cm.Title == "Curacha" && cm.Description == "Curacha got its name because of its spiky, hairy appearance that kinda resembles you-know-what.");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("CR").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("CRNonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("CR");
