...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class Garden_Orchid_Hotel : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_garden_orchid_hotel);
            ...
            {
                Resource.Drawable.garden_orchid_hotel_1,
                Resource.Drawable.garden_orchid_hotel_2,
                Resource.Drawable.garden_orchid_hotel_3,
                Resource.Drawable.garden_orchid_hotel_4,
            };
            ...
            else
            {
                favoritesItem = new FavoriteItem
                {
                    ...
                    Title = "Garden Orchid Hotel",
                    Description = "It is a luxurious hotel located in Governor Camins Avenue, Zamboanga City. The hotel offers a variety of rooms, ranging from deluxe rooms to suites, with rates starting at around PHP 4,000 per night."
                };
            }
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "garden_orchid_hotel_1" && cm.Title == "Garden Orchid Hotel" && cm.Description == "It is a luxurious hotel located in Governor Camins Avenue, Zamboanga City. The hotel offers a variety of rooms, ranging from deluxe rooms to suites, with rates starting at around PHP 4,000 per night.");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("GOH").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("GOHNonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("GOH");
