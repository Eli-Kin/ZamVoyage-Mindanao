...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class Tricycle : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_tricycle);
            ...
            else
            {
                favoritesItem = new FavoriteItem
                {
                    ImagePath = "tricycle",
                    Title = "Tricycle",
                    Description = "The fare for a tricycle ride is usually negotiated with the driver, and it's important to agree on the fare before starting the ride. "
                };
            }
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "tricycle" && cm.Title == "Tricycle" && cm.Description == "The fare for a tricycle ride is usually negotiated with the driver, and it's important to agree on the fare before starting the ride. ");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("TRI").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("TRINonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("TRI");
