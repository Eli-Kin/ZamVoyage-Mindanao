...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class Bus_Terminals : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_bus_terminals);
            ...
            else
            {
                favoritesItem = new FavoriteItem
                {
                    ImagePath = "bus",
                    Title = "Bus Terminals",
                    Description = "The terminal has several bus bays and ticketing counters. The terminal serves several bus companies that operate routes to other cities in Mindanao, including Dapitan, Dipolog, Pagadian, and Cotabato. "
                };
            }
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "bus" && cm.Title == "Bus Terminals" && cm.Description == "The terminal has several bus bays and ticketing counters. The terminal serves several bus companies that operate routes to other cities in Mindanao, including Dapitan, Dipolog, Pagadian, and Cotabato. ");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("BUS").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("BUSNonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("BUS");
