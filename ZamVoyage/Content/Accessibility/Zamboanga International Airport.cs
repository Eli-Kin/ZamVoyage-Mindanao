...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class Zamboanga_International_Airport : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_zamboanga_international_airport);
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "airplane" && cm.Title == "Zamboanga International Airport" && cm.Description == "The airport serves the Zamboanga Peninsula region and is a hub for several airlines, including Cebu Pacific, Philippine Airlines, and AirAsia. ");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("AIR").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("AIRNonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("AIR");
