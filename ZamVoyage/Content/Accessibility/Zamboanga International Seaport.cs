...
namespace ZamVoyage.Content.Mountains
{
    ...
    public class Zamboanga_International_Seaport : AppCompatActivity, Android.Gms.Tasks.IOnSuccessListener
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.layout_zamboanga_international_seaport);
        ...
        private int GetContentIdFromDatabase()
        {
            ...
            FavoriteItem favoriteContent = favoritesItems.FirstOrDefault(cm => cm.ImagePath == "ship" && cm.Title == "Zamboanga International Seaport" && cm.Description == "The seaport is one of the busiest in the region and serves as a gateway to nearby destinations such as Basilan, Jolo, and Tawi-Tawi. ");
        ...
        protected override void OnPause()
        {
            ...
            if (firebaseAuth.CurrentUser != null)
            {
                ...
                firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("SHIP").Get().AddOnSuccessListener(this);
            }
            else
            {
                // No user is logged in, store the toggle state for non-logged-in user
                ISharedPreferences prefs = GetSharedPreferences("SHIPNonLoggedInToggleState", FileCreationMode.Private);
        ...
        private async void AddToFavoritesFirestore()
        {
            try
            {
                ...
                DocumentReference document = firestore.Collection("users").Document(firebaseAuth.CurrentUser.Uid).Collection("favorites").Document("SHIP");
